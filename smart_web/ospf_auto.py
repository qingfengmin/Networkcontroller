...
class ospf_auto:
    def __init__(self):
        #无需导入直接读取配置文件中的设备信息
        self.device = rf().templist
        self.con = con()

    def ospf_web(self,process_id,area_id):
        #用于web界面配置OSPF的方法
    ...
    def __ospf_process(self, process_id, area_id):
        ...
        for device in self.device.keys():
            interfaces = ['loopback0'] + [f'vlanif{vlan}' for vlan in self.device[device]['vlanif'].keys()]
            router_id = self.device[device]['loopback_ip']
