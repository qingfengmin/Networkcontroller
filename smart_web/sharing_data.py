...
class database:
    def __init__(self):
        #调入settings,用于保证GUI传入的VLAN,LOOPBACK,BD,VNI等信息共享
        self.Devices = {}
        ...
        self.__loopback_network = list(self.settings.loopback().keys())
    ...
    def create_device(self, host, device_type):
        ...
        try:
            ...
            self.rf.write_host(self.hostlist)
            #将设备信息写入文件,在配置文件中保存一份
    ...
    def delete_device(self, host):
        ...
        if host in self.hostlist:
            ...
            self.rf.detlete(host)
            #删除设备信息,并将文件中的设备信息删除


    def init(self):
        #设备初始化方法,用于获取设备的信息,设备基于这些信息进行配置
        hostlist = rf().hostlist.keys()
        ...
        self.rf.write(self.Devices)
        #将计算好的设备信息写入文件中以便让后来的PY文件使用
    ...
    def __vlan_format(self, vlan_list):
        # 用于格式化 VLAN 列表的函数,由于华为设备的VLAN仅支持2进制,所以需要格式化传入
    ...
    def __generate_vlan_config(self, lldp_info):
        #基于LLDP信息生成VLAN配置的函数,传入LLDP信息,返回VLAN配置
        # 直接获取最新配置
