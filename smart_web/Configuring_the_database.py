...
class config:
    ...
    def interface_addrsss(self, interface,network,mask):
        #配置接口地址的函数,指定参数可以返回一个配置接口地址的xml
        looback = f'''
	     <config>
...
'''
    ...
    def vxlan_BD(self, bd_id, vni):
        #用于创建VXLAN,BD域及VNI以及使能动态VXLAN隧道的函数
        vxlan_core = f'''
    <config>
...
'''
    ...
    def bgp_neighbor(self, address,fixed_as, interface):
        #建立BGP对等体并配置邻居地址和AS号,以及使能EVPN,ARP路由
        bgp_neighbor = f'''
    <config>
      <bgp xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
        <bgpcomm>
...
#一些初始化配置相关的函数,如使能LLDP,EVPN,创建NVE接口,也存在XML过滤器
generic = {'int_filter':'''
    <filter type="subtree">
      <ifm xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
        <interfaces>
