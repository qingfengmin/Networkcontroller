...
class vxlan_auto:
    def __init__(self):
        ...
        self.rf = rf()
        self.device = self.rf.templist
        self.host = self.rf.hostlist
        self.bd_list = list(setting().BD())
        self.vni_list = list(setting().vni())
        self.con = con()

    def bgp_peer_web(self,as_number):
        #用于web界面配置BGP的方法
        self.__bgp_peer(as_number)

    def vxlan_gateway_web(self,bd_id,vni,RD,import_rt,export_rt,address,mask):
        self.__vxlan_tunnel(bd_id,vni)
        self.__tunnel(address,mask,bd_id,RD,export_rt,import_rt)
    ...
    def vxlan_gateway_gui(self, parent_window):
        ...
        def on_configure():
            ...
            self.__tunnel(address,mask,bd,rd,export_rt,import_rt)
